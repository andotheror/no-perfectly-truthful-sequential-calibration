...
\[
\E_{Y\sim\Ber(1/2)}\ell_{\beta}(Y)\le 2h(1/2).
\]
...
Write \(\{0,1\}^{<T}=\bigcup_{t=0}^{T-1}\{0,1\}^t\), and let \(x_{<t}=(x_1,\ldots,x_{t-1})\). A deterministic forecaster is a binary probability tree \(A=(a_u)_{u\in\{0,1\}^{<T}}\), where \(a_u\in[0,1]\) is the prediction after history \(u\). On outcome sequence \(x\in\{0,1\}^T\), its realized prediction vector is
\[
A(x)=(a_{x_{<1}},\ldots,a_{x_{<T}}).
\]
A calibration measure is a family \(M=(M_T)_{T\ge1}\) with \(M_T:\{0,1\}^T\times[0,1]^T\to[0,T]\). Its expected error under outcome distribution \(P\) and strategy \(A\) is
\[
\operatorname{err}_{M_T}(P,A)=\E_{X\sim P}M_T(X,A(X)).
\]

For a full-support \(P\in\simplex(\{0,1\}^T)\), its truthful tree has node reports
\[
...
The measure \(M\) is perfectly truthful if the truthful tree minimizes expected error for every \(T\) and every \(P\). It is deterministically complete if \(M_T(x,x)=0\) for all \(T,x\). It is product-complete if, for every fixed \(\alpha\in[0,1]\),
\[
\E_{X\sim\Ber(\alpha)^{\otimes T}}M_T(X,\alpha\1)=o_\alpha(T).
\]
It is product-sound if, for every fixed \(\alpha\ne\beta\),
\[
\E_{X\sim\Ber(\alpha)^{\otimes T}}M_T(X,\beta\1)=\Omega_{\alpha,\beta}(T).
\]
\end{definition}
...
\[
S_T(Q,x)=M_T(x,A_Q(x)),\qquad H_T(P)=\E_{X\sim P}S_T(P,X).
\tag{1}
\]
Perfect truthfulness says that \(S_T\) is a proper scoring rule:
\[
H_T(P)\le \E_{X\sim P}S_T(Q,X)
\tag{2}
\]
...
Fix \(T\) and a bounded perfectly truthful realized-path score. There are bounded concave functions \(h_{T,u}:[0,1]\to\mathbb R\), one for every \(u\in\{0,1\}^{<T}\), such that every full-support distribution \(P\) satisfies
\[
H_T(P)=\sum_{u\in\{0,1\}^{<T}}P(u)h_{T,u}(q_u).
\tag{3}
\]
If the score is nonnegative and deterministically complete, the functions can be chosen so that
\[
h_{T,u}(0)=h_{T,u}(1)=0,\qquad h_{T,u}(q)\ge0.
\tag{4}
\]
...
\[
S_T(Q,x)=\sum_{u\prec x}\left[h_{T,u}(q_u)+(x_{|u|+1}-q_u)h_{T,u}'(q_u)\right].
\tag{5}
\]
\end{theorem}
...
For \(0<p<q<1\), properness at true parameters \(p\) and \(q\) traps the increment of \(\ell_0\):
\[
\frac{p}{1-p}\bigl(\ell_1(p)-\ell_1(q)\bigr)
\le \ell_0(q)-\ell_0(p)
\le \frac{q}{1-q}\bigl(\ell_1(p)-\ell_1(q)\bigr).
\tag{6}
\]
The same interval traps the increment of \(\widetilde\ell_0\). On a refining partition, the sum of the interval widths tends to zero because \(\ell_1\) is bounded and monotone. Thus the two increments agree. Appendix~\ref{app:shared} gives the details.

To see how Lemma~\ref{lem:shared} yields Theorem~\ref{thm:factorization}, split a leaf distribution at the root as
\[
P=((1-a)P_0,aP_1).
\]
...
\[
p\mapsto\bigl(K_0^p(R_0),K_1^p(R_1)\bigr)
\]
is a proper binary loss for every \(R_0,R_1\). Holding one conditional fixed and applying Lemma~\ref{lem:shared} gives
\[
K_b^p(R)=a_b(p)+K_b(R).
\tag{7}
\]
...
\[
0\le H_T(P)\le T(1-P(x))\longrightarrow0.
\tag{8}
\]
...
Let \(h:[0,1]\to\mathbb R_+\) be concave with \(h(0)=h(1)=0\), and suppose \(h\) is differentiable at \(p\in(0,1)\). Define
\[
\ell_p(b)=h(p)+(b-p)h'(p),\qquad b\in\{0,1\}.
\]
For every \(q\in(0,1)\),
\[
(1-q)\ell_p(0)+q\ell_p(1)\le C_{p,q}h(q),
\tag{9}
\]
where
\[
C_{p,q}=\max\left\{\frac q p,\frac{1-q}{1-p}\right\}
\max\left\{\frac p q,\frac{1-p}{1-q}\right\}.
\]
\end{lemma}

The tangent line lies above the zero endpoint values, hence \(\ell_p(0),\ell_p(1)\ge0\). Reweighting these two nonnegative numbers gives
\[
(1-q)\ell_p(0)+q\ell_p(1)
\le \max\left\{\frac q p,\frac{1-q}{1-p}\right\}h(p).
\]
...
\begin{proof}
Apply Theorem~\ref{thm:factorization} at every horizon. Each concave node entropy is nondifferentiable at at most countably many points. The union of these exceptional sets over all nodes and horizons is countable. Choose one fixed
\[
...
\]
Product completeness makes the last expectation \(o(T)\). Product soundness requires the first expectation to be \(\Omega(T)\), a contradiction.
\end{proof}
...
For every bounded, perfectly truthful, deterministically complete family \(M\), there is a fixed \(\beta\in(1/3,2/3)\setminus\{1/2\}\) such that, for every \(T\),
\[
\E_{P_{1/2}}M_T(X,\beta\1)\le2\E_{P_{1/2}}M_T(X,\tfrac12\1).
\]
\end{corollary}
...
\[
\mu(A)=\E_{Y\sim Q_A}\sum_{t=1}^T Y_t,
\qquad
M_T^{\mathrm{tree}}(x,A)=\frac1T\left(\sum_{t=1}^T x_t-\mu(A)\right)^2.
\tag{10}
\]
...
\begin{proof}
For true distribution \(P\), write \(Z=\sum_tX_t\). Then
\[
\E_P M_T^{\mathrm{tree}}(X,A)
=\frac{\operatorname{Var}_P(Z)}{T}
+\frac{(\E_PZ-\mu(A))^2}{T}.
\]
The truthful tree induces \(P\) and reports \(\mu(A_P)=\E_PZ\), so it is optimal. For deterministic \(P=\delta_x\), its score is zero. Under \(P_\alpha=\Ber(\alpha)^{\otimes T}\), the truthful expected score is \(\alpha(1-\alpha)\). The constant tree \(A\equiv\beta\) has expected score
\[
...
\[
\E_{Y\sim P}S(P,Y)\le \E_{Y\sim P}S(Q,Y)
\]
for all \(P,Q\in\simplex(\mathcal Y)\). Its entropy is \(H(P)=\E_PS(P,Y)\). Since
\[
H(P)=\inf_Q\E_PS(Q,Y),
\]
...
\[
H(P)\le \sum_yP(y)S(Q,y),
\qquad
H(Q)=\sum_yQ(y)S(Q,y).
\tag{11}
\]
Conversely, a choice of supporting affine majorant at every \(Q\) defines a proper score. This is the finite-outcome Savage representation \citep{gneiting2007strictly,dawid2012local}.
...
\[
\overline H(z)=\left(\sum_yz_y\right)H\left(\frac{z}{\sum_yz_y}\right).
\]
The score vector is a supergradient of the concave one-homogeneous function \(\overline H\). At differentiability points it equals \(\nabla\overline H(Q)\).

\section{Proof of shared-side uniqueness}\label{app:shared}

We prove Lemma~\ref{lem:shared}. A bounded pair \((\ell_0,\ell_1)\) is proper when, for all \(p,q\in(0,1)\),
\[
(1-q)\ell_0(q)+q\ell_1(q)
\le(1-q)\ell_0(p)+q\ell_1(p).
\tag{12}
\]
Fix \(0<p<q<1\). Apply Equation~(12) first with true parameter \(p\) and report \(q\), then with true parameter \(q\) and report \(p\). With
\[
\Delta_0=\ell_0(q)-\ell_0(p),
\qquad
\Delta_1=\ell_1(p)-\ell_1(q),
\]
the two inequalities become
\[
\frac{p}{1-p}\Delta_1\le\Delta_0\le\frac{q}{1-q}\Delta_1.
\tag{13}
\]
The interval is nonempty only if \(\Delta_1\ge0\), and then \(\Delta_0\ge0\). Thus \(\ell_0\) is nondecreasing and \(\ell_1\) is nonincreasing.
...
For \(T\ge2\), split the leaf space into \(\mathcal Y_0=\{x:x_1=0\}\) and \(\mathcal Y_1=\{x:x_1=1\}\). Parameterize a full-support report distribution as
\[
Q=((1-p)Q_0,pQ_1).
\]
For \(x\in\mathcal Y_b\), realized-path locality implies
\[
S_T(Q,x)=S_b(p,Q_b,x),
\tag{14}
\]
...
\[
\E_{R_b}S_b(p,R_b,Y)\le\E_{R_b}S_b(p,Q_b,Y).
\]
Thus \(S_b(p,\cdot,\cdot)\) is a bounded proper score on \(\mathcal Y_b\). Denote its entropy by
\[
K_b^p(R_b)=\E_{Y\sim R_b}S_b(p,R_b,Y).
\]
...
\[
(1-a)K_0^a(R_0)+aK_1^a(R_1)
\le(1-a)K_0^p(R_0)+aK_1^p(R_1).
\tag{15}
\]
Hence \(p\mapsto(K_0^p(R_0),K_1^p(R_1))\) is a bounded proper binary loss for every pair \(R_0,R_1\).

Fix a reference \(R_0^\circ\). Holding \(R_1\) fixed in Equation~(15), Lemma~\ref{lem:shared} shows that
\[
...
\[
h_{T,\varnothing}(a)=(1-a)a_0(a)+aa_1(a)
\]
is concave.

Choose any \(p_0\in(0,1)\). The branch score
\[
\widetilde S_b(Q_b,y)=S_b(p_0,Q_b,y)-a_b(p_0)
\]
...
\[
0\le H_T(P)\le\E_PM_T(X,A^x(X))\le T(1-P(x)).
\tag{16}
\]
...
\[
\sum_{u\prec x}e_{u,x_{|u|+1}}=0
\tag{17}
\]
for every leaf \(x\).

Set \(c_\varnothing=0\) and recursively define \(c_{ub}=c_u-e_{u,b}\). Equation~(17) gives \(c_x=0\) at every leaf. Replace
\[
h_{T,u}(q)
\quad\text{by}\quad
\widetilde h_{T,u}(q)=h_{T,u}(q)+(1-q)c_{u0}+qc_{u1}-c_u.
\tag{18}
\]
...
\[
h_{T,u}(q_u)+(b-q_u)h_{T,u}'(q_u).
\]
It has derivative zero for leaves outside the subtree. Summing over nodes gives the right side of Equation~(5). By Appendix~\ref{app:proper}, this derivative is the unique supporting score at \(Q\), so it equals the actual \(S_T(Q,x)\).
...
\tag{19}
\end{align*}
If \(p\le q\), write \(q=\lambda p+(1-\lambda)1\), where \(\lambda=(1-q)/(1-p)\). Concavity gives
\[
h(q)\ge\frac{1-q}{1-p}h(p).
\]
If \(p\ge q\), write \(q=\lambda p+(1-\lambda)0\), where \(\lambda=q/p\), to obtain \(h(q)\ge(q/p)h(p)\). In both cases,
\[
h(p)\le\max\left\{\frac{1-p}{1-q},\frac p q\right\}h(q).
\tag{20}
\]
Equations~(19) and (20) prove Lemma~\ref{lem:tangent}.
...
\]
Theorem~\ref{thm:factorization} says that its entropy must be
\[
...
\[
\frac1T\left(\sum_{t=1}^T(x_t-p)\right)^2
=T(\overline x-p)^2
\]
